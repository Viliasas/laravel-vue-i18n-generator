...
namespace Viliasas\VueInternationalizationGenerator;
...
class Generator
{
    ...
    private function allocateLocaleArray($path, $multiLocales = false)
    {
        ...
        foreach ($dir as $fileinfo) {
            ...
            if ($fileinfo->isDir()) {
                ...
                $data[$fileinfo->getFilename()] = $this->allocateLocaleArray(
                ...
                if ($lastLocale !== false) {
                    ...
                    $filePath = $this->removeExtension(
                        str_replace('\\', '_', ltrim(str_replace($root, '', realpath($fileName)), '\\'))
                    );
    ...
    private function shouldIgnoreLangFile($noExt)
    {
        ...
        return (isset($this->config['langFiles']) && !empty($this->config['langFiles']) && !in_array(
                    $noExt,
